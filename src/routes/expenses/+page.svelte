<script lang="ts">
	...
	function onKeydown(event: KeyboardEvent): void {
		if (event.key === 'Escape' && filterOpen) filterOpen = false;
	}
	...
	<div class="ds-content">
		...
		{#each groups as group (group.key)}
			...
			{#each group.items as tx (tx.id)}
				<div class="ds-transaction mb-3">
					<span class="ds-icon-avatar ds-icon-avatar-sm {tx.tint}"
						><i class="bi {tx.icon}"></i></span
					>
					<div>
						<div class="name">{tx.name}</div>
						<div class="category">{tx.category}</div>
					</div>
					<span class="amount" class:over={overBudget && tx.category === 'Shopping'}
						>- {formatRp(tx.amount)}</span
					>
				</div>
			{/each}
		{/each}
...
{#if filterOpen}
	...
	<div
		...
		style="display:block;"
		onclick={(e) => e.target === e.currentTarget && (filterOpen = false)}
	>
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header border-0 pb-2">
					<h5 class="modal-title fw-bold" id="filterModalTitle">Filter Transactions</h5>
					<button
						type="button"
						class="btn-close"
						aria-label="Close"
				...
				<form novalidate onsubmit={applyFilter}>
					...
					<div class="modal-footer border-0 pt-0">
						<button type="button" class="btn ds-btn-neutral-tint" onclick={resetFilter}
							>Reset</button
						>
